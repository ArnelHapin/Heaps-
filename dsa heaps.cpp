...
// MaxHeap class to manage heap operations for swords
class MaxHeap {
private:
    ...
    void heapifyDown(int i) {
        ...
        // Check if left child exists and has higher attack power
        if (leftChild < heap.size() && heap[leftChild].attackPower > heap[largest].attackPower) {
            largest = leftChild;
        }

        // Check if right child exists and has higher attack power
        if (rightChild < heap.size() && heap[rightChild].attackPower > heap[largest].attackPower) {
            largest = rightChild;
        }
...
public:
    ...
    // Delete the root sword (the sword with the highest attack power)
    void deleteRoot() {
        if (heap.size() == 0) {
    ...
    // Convert an unsorted array of swords into a Max-Heap
...
int main() {
    ...
    swordHeap.insert(Sword(80, 80));    // Sword with attack power 80, weight 8
    ...
    // Delete the root (the sword with the highest attack power)
